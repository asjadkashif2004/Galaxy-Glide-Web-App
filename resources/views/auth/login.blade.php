...
<style>
  ...
  /* Fixed full-viewport starfield behind everything */
  .space-bg{
    position: fixed;
    inset: 0;
    z-index: 0;              /* content sits above (z-index:1) */
    pointer-events: none;
    overflow: hidden;
    background:
  ...
  .auth-wrap{
    ...
    background: transparent; /* let the global space sky show through */
    color:var(--text); overflow:hidden; padding: 20px 12px;
  }

  /* Keep your layered local stars too (extra depth). Optional—keep or remove. */
  .stars, .stars2, .stars3{
    position:absolute; inset:0; background-repeat:repeat; background-position:0 0; pointer-events:none;
  }
  .stars  { background-image:radial-gradient(2px 2px at 20px 30px,rgba(255,255,255,.8),transparent 40%),radial-gradient(1.5px 1.5px at 120px 80px,rgba(255,255,255,.6),transparent 40%),radial-gradient(2px 2px at 200px 20px,rgba(255,255,255,.75),transparent 40%); background-size: 400px 400px; animation: drift 120s linear infinite; opacity:.35;}
  .stars2 { background-image:radial-gradient(1.3px 1.3px at 60px 90px,rgba(255,255,255,.7),transparent 40%),radial-gradient(1.3px 1.3px at 160px 190px,rgba(255,255,255,.5),transparent 40%),radial-gradient(1.3px 1.3px at 300px 40px,rgba(255,255,255,.65),transparent 40%); background-size: 600px 600px; animation: drift 180s linear infinite reverse; opacity:.3;}
  .stars3 { background-image:radial-gradient(1px 1px at 90px 140px,rgba(255,255,255,.6),transparent 40%),radial-gradient(1px 1px at 260px 240px,rgba(255,255,255,.45),transparent 40%),radial-gradient(1px 1px at 380px 120px,rgba(255,255,255,.5),transparent 40%); background-size: 800px 800px; animation: drift 240s linear infinite; opacity:.25;}
  @keyframes drift{ 0%{ transform: translateY(0) } 100%{ transform: translateY(-400px) } }

  .shooting-star{ position:absolute; top:10%; left:-10%; width:2px; height:2px; background:transparent; opacity:0; transform:translate3d(0,0,0) rotate(35deg); animation: shoot 8s ease-in-out infinite; }
  .shooting-star::after{ content:""; position:absolute; width:140px; height:2px; right:-6px; top:0; background:linear-gradient(90deg, rgba(255,255,255,.0), rgba(255,255,255,.9), rgba(56,189,248,.0)); filter: drop-shadow(0 0 6px #fff); }

  .card{
    position:relative; width:min(960px, 92vw); display:grid; grid-template-columns: 1.1fr .9fr;
  ...
  .aura{ position:absolute; inset:-60%; pointer-events:none; opacity:.35;
    background: radial-gradient(600px 320px at 80% 20%, rgba(56,189,248,.35), transparent 70%), radial-gradient(600px 320px at 10% 90%, rgba(99,102,241,.35), transparent 70%), radial-gradient(700px 380px at 50% 50%, rgba(6,182,212,.25), transparent 70%);
    filter: blur(40px);
  }

  .panel-art{
    position:relative; border-radius:var(--radius-sm); border:1px solid var(--border); overflow:hidden;
    background: radial-gradient(900px 500px at -10% 110%, rgba(56,189,248,.1), transparent 60%), linear-gradient(180deg, rgba(3,7,18,.8), rgba(2,6,23,.65));
  ...
  .logo{ width:44px; height:44px; border-radius:12px; display:grid; place-items:center; overflow:hidden; background: conic-gradient(from 220deg, var(--accent-1), var(--accent-2), var(--accent-3)); box-shadow: 0 6px 18px rgba(56,189,248,.35); }
  ...
  .panel-form{ position:relative; border-radius:var(--radius-sm); border:1px solid var(--border);
  ...
  .input{ width:100%; border-radius:12px; border:1px solid var(--border); background: rgba(255,255,255,.03); color:var(--text);
  ...
  .check input{ appearance:none; width:18px; height:18px; border-radius:6px; border:1px solid var(--border); background:rgba(255,255,255,.04); position:relative; outline:none; cursor:pointer; transition: all .2s; }
  ...
  .btn{ display:inline-flex; align-items:center; gap:8px; border:0; cursor:pointer; border-radius:12px; padding: 11px 14px; color:#0b1220; background:white; font-weight:700; font-size:.95rem; transition: transform .05s ease, box-shadow .2s ease, opacity .2s ease; }
...
<!-- Fixed animated background (global for this page) -->
...
<div class="page-fore">
  <div class="auth-wrap">
    <!-- local layered parallax (optional depth) -->
    <div class="stars"></div>
    ...
    <div class="card">
      ...
      <section class="panel-art" aria-label="NASA Zoom Explorer brand">
        <div class="brand">
          ...
          <div>
            ...
            <div class="sub">Welcome back, explorer.</div>
          </div>
        </div>
      ...
      <section class="panel-form" aria-label="Sign in form">
        <header class="heading">
          <h2>Log in to your mission control</h2>
          <p>Access dashboards, missions, and cosmic archives.</p>
        </header>
        ...
        <form method="POST" action="{{ route('login') }}" novalidate>
          ...
          <div class="field">
            ...
            <input id="email" class="input" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username" placeholder="you@universe.space">
            @if ($errors->has('email'))
              <div class="error">
                <ul>
          ...
          <div class="micro">Protected portal for authorized astronomers and mission specialists.</div>
        </form>
      </section>
    </div>
  </div>
